...
import { imageConfig } from '../constants/image.variant';

describe('ymlwebcl-image', () => {
  const IMAGE_COMPONENT = 'ymlwebcl-image';
  let imageElement: ImageComponent;
  ...
  it('sets image max-height property', async () => {
    ...
    await imageElement.updateComplete;
    const imageEle = getShadowRoot(IMAGE_COMPONENT).querySelector('img');
    expect(imageEle.getAttribute('max-height')).toBe(
      imageConfig.size.maxHeight
    );
  });
  ...
  it('sets image min-height property', async () => {
    ...
    await imageElement.updateComplete;
    const imageEle = getShadowRoot(IMAGE_COMPONENT).querySelector('img');
    expect(imageEle.getAttribute('min-height')).toBe(
      imageConfig.size.minHeight
    );
  });
  ...
  it('sets image border property', async () => {
    ...
    await imageElement.updateComplete;
    const imageEle = getShadowRoot(IMAGE_COMPONENT).querySelector('img');
    expect(imageEle.style.border).toContain(
      imageConfig.size.border.width + ' ' + imageConfig.size.border.type
    );
  });
